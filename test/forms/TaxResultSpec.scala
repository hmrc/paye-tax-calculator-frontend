...
class TaxResultSpec extends BaseSpec with AnyWordSpecLike {
  ...
  "Extracting Salary from user response" should {
    ...
    "return if response provided is the Yearly Salary: £20000" in {
      extractSalary(QuickCalcAggregateInput(Some(Salary(20000, "a year", None)), None, None, None, None)) mustBe 20000
    }

    "return if response provided is the Monthly Salary: £2000" in {
      extractSalary(QuickCalcAggregateInput(Some(Salary(2000, "a month", None)), None, None, None, None)) mustBe 2000
    }

    "return if response provided is the Weekly Salary: £200" in {
      extractSalary(QuickCalcAggregateInput(Some(Salary(200, "a week", None)), None, None, None, None)) mustBe 200
    }

    "return if response provided is the Daily Salary: £20" in {
      extractSalary(QuickCalcAggregateInput(Some(Salary(20, "a day", None)), None, None, None, None)) mustBe 20
    }

    "return if response provided is the Hourly Salary: £2 in pence" in {
      extractSalary(QuickCalcAggregateInput(Some(Salary(2, "an hour", None)), None, None, None, None)) mustBe 2
    }
  ...
  "Extracting Pay Period from user response" should {
    ...
    "return  if response provided is Yearly" in {
      extractPayPeriod(QuickCalcAggregateInput(Some(Salary(0, "a year", None)), None, None, None, None)) mustBe PayPeriod.YEARLY
    }

    "return if response provided is Monthly" in {
      extractPayPeriod(QuickCalcAggregateInput(Some(Salary(0, "a month", None)), None, None, None, None)) mustBe PayPeriod.MONTHLY
    }

    "return if response provided is Weekly" in {
      extractPayPeriod(QuickCalcAggregateInput(Some(Salary(0, "a week", None)), None, None, None, None)) mustBe PayPeriod.WEEKLY
    }

    "return empty string if response is Daily" in {
      extractPayPeriod(QuickCalcAggregateInput(Some(Salary(0, "a day", None)), None, None, None, None)) mustBe PayPeriod.DAILY
    }

    "return empty string if response is Hourly" in {
      extractPayPeriod(QuickCalcAggregateInput(Some(Salary(0, "an hour", None)), None, None, None, None)) mustBe PayPeriod.HOURLY
    }

    "return empty string if response is Every 4 weeks" in {
      extractPayPeriod(QuickCalcAggregateInput(Some(Salary(0, "every 4 weeks", None)), None, None, None, None)) mustBe PayPeriod.FOUR_WEEKLY
    }
  ...
  "Extracting Hours from user response" should {
    ...
    "return if response is hours in Daily" in {
      extractHours(QuickCalcAggregateInput(Some(Salary(40, "a day", Some(4.0))), None, None, None, None)) mustBe Some(
        4.0
      )
    }

    "return if response is hours in Hourly" in {
      extractHours(QuickCalcAggregateInput(Some(Salary(20, "an hour", Some(10.0))), None, None, None, None)) mustBe Some(
        10.0
      )
    }

    "return if response is not Daily or Hourly" in {
      extractHours(QuickCalcAggregateInput(Some(Salary(-1, "an hour", None)), None, None, None, None)) mustBe None
    }
