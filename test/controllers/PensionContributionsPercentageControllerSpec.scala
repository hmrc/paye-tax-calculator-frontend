  ...
  "Show Pension Contributions Form" should {
    ...
    "return 200 and a list of current aggregate data containing Tax Code and pension and tax code answered" in {
      ...
      running(application) {
        ...
        val formFilled = form.fill(cacheSalaryTaxCodeSavedPensionContributionsPercentage.value.savedPensionContributions.get)

        status(result) mustEqual OK

        removeCSRFTagValue(contentAsString(result)) mustEqual removeCSRFTagValue(
          view(formFilled,
            cacheSalaryTaxCodeSavedPensionContributionsPercentage.value.additionalQuestionItems()(messages, mockAppConfig))(
            request,
  ...
  "Submit Pension Contributions Form" should {
    "return 400, with aggregate data and an error message for invalid pension amount" in {
      ...
      running(application) {
        ...
        val formData = Map("gaveUsPensionPercentage" -> "true", "monthlyPensionContributions" -> "40;")
    ...
    "return 400, with no aggregate data and an error message for pension contributions when value is more than 2dp" in {
      ...
      running(application) {
        ...
        val formData = Map("gaveUsPensionPercentage" -> "true", "monthlyPensionContributions" -> "40.023")
