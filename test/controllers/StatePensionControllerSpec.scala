...
class StatePensionControllerSpec
  ...
  "Show State Pension Form" should {
    ...
    "Submit State Pension Form" should {
      ...
      "return 303, with an answer \"No\" saved on the current list of aggregate data of all answered questions and redirect to Summary-Answers" in {
        val mockCache = mock[QuickCalcCache]
        ...
        running(application) {
          ...
          redirectLocation(result).value mustEqual routes.QuickCalcController.summary()
            .url
          verify(mockCache, times(1)).fetchAndGetEntry()(any())
        }
      }
