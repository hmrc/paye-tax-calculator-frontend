...
class ScottishWinterFuelPaymentsControllerSpec extends PlaySpec with TryValues with ScalaFutures with IntegrationPatience with MockitoSugar with CSRFTestHelper {
  ...
  "Show Winter Fuel Payment page" should {
    ...
    def showWinterFuelPaymentPage(lang: String = "en") = {
      ...
      running(application) {
        ...
        val request = FakeRequest(GET, routes.ScottishWinterFuelPaymentsController.showScottishWinterFuelPayments().url)
        ...
        content must include(messages("quick_calc.scottish_wfp_subheading1"))
        content must include(messages("quick_calc.scottish_wfp_subheading2"))
        content must include(messages("quick_calc.scottish_wfp_subheading3"))
        content must include(messages("quick_calc.scottish_wfp_subheading4"))
        if (lang == "cy")
