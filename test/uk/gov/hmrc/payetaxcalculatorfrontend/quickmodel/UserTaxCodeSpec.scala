...
class UserTaxCodeSpec extends BaseSpec {
  ...
  "The checkUserSelection function" should {
    ...
    "set the yes option as checked, if user has checked yes in earlier operation" in {
      UserTaxCode
        .checkUserSelection(true, userTaxCodeForm.fill(UserTaxCode(true, Some(UserTaxCode.defaultUkTaxCode)))) shouldBe "checked"
    }
    "set the no option as checked, if user has checked no in earlier operation" in {
      UserTaxCode.checkUserSelection(
        false,
        userTaxCodeForm.fill(UserTaxCode(false, Some(UserTaxCode.defaultUkTaxCode)))
      ) shouldBe "checked"
    }
  }
