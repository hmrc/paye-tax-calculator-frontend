...
//    }
//
//    "do not override a sessionId if one already exist" in {
//
...
//    }
//
//    "do not override other session values from the response" in {
//
//      val Some(result) = route(app, FakeRequest(GET, "/test2"))
//      print(session(result).data)
//      session(result).data.get("foo") mustBe Some("bar")
//    }
//
//    "do not override other session values from the request" in {
//
...
import play.api.inject.guice.GuiceApplicationBuilder
import play.api.libs.json.Json
import play.api.mvc.{Action, DefaultActionBuilder, Results, SessionCookieBaker}
import play.api.routing.Router
import play.api.test.FakeRequest
import play.api.test.Helpers._
import uk.gov.hmrc.http.{HeaderNames, SessionKeys}
